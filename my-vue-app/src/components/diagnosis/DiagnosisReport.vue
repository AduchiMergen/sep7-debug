<template>
  <div class="diagnosis-report">
    <h1 class="text-2xl font-bold mb-2">
      SEP-7 URI Diagnosis Report
    </h1>
    <p class="text-sm text-gray-600 mb-1">
      <strong>URI:</strong> <code class="break-all">{{ reportData.uri }}</code>
    </p>
    <p class="text-sm text-gray-500 mb-6">
      <strong>Analyzed on:</strong> {{ new Date(reportData.timestamp).toLocaleString() }}
    </p>

    <!-- Syntax Analysis Card -->
    <DiagnosticCard
      title="Syntax Analysis"
      :status="reportData.syntax?.isValid"
    >
      <div v-if="reportData.syntax">
        ...
        <div v-if="!reportData.syntax.isValid && reportData.syntax.errors.length > 0">
          <h4 class="font-semibold text-red-600 mt-2">
            Errors:
          </h4>
          <ul class="list-disc list-inside ml-4 text-red-500">
            <li
              v-for="(err, i) in reportData.syntax.errors"
              :key="i"
            >
              {{ err.message }}
            </li>
        ...
        <p
          v-else-if="reportData.syntax.isValid"
      ...
      <p
        ...
        class="text-gray-500"
      >
        No syntax analysis data.
      </p>
    </DiagnosticCard>

    <!-- Parameters Analysis Card -->
    <DiagnosticCard
      title="Parameters Analysis"
      v-if="reportData.parameters && reportData.syntax?.isValid"
    >
      <ParamsTable
    ...
    >
      <p class="text-gray-500">
        Skipped due to invalid URI syntax.
      </p>
    </DiagnosticCard>
    ...
    <DiagnosticCard
      ...
      :status="reportData.security?.summary"
      v-if="reportData.security && reportData.syntax?.isValid"
    >
      <div class="space-y-2">
        <p>
          ...
          >Not Provided</span>
          <span
            v-if="reportData.security.originDomain?.value"
            :class="['ml-2 text-xs', reportData.security.originDomain.isValid ? 'text-green-600' : 'text-red-600']"
          >
        ...
        <p>
          ...
          <span
            v-if="reportData.security.signature?.value"
            class="truncate inline-block max-w-md"
          >{{ reportData.security.signature.value }}</span>
          <span
            v-else
            class="text-gray-500"
          >Not Provided</span>
          <span
            v-if="reportData.security.signature?.value"
            :class="['ml-2 text-xs', reportData.security.signature.isValid ? 'text-green-600' : 'text-yellow-600']"
          >
        ...
        <div v-if="reportData.security.messages && reportData.security.messages.length > 0">
          ...
          <ul class="list-disc list-inside ml-4 text-yellow-600">
            <li
              v-for="(msg, i) in reportData.security.messages"
              :key="i"
            >
              {{ msg.message }}
            </li>
        ...
        <p class="mt-2">
          <strong>Overall Security Assessment:</strong> <span :class="getMessageClass(reportData.security.summary)">{{ reportData.security.summary }}</span>
        </p>
      </div>
    </DiagnosticCard>
    <DiagnosticCard
